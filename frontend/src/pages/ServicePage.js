...
const serviceImages = {};
const imageModules = require.context('../assets', false, /service-.*\.jpg$/);
imageModules.keys().forEach((key) => {
  const name = key.replace('./', '').replace('.jpg', '');
  serviceImages[name] = imageModules(key);
});
...
const ServicePage = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-industrial-white">
        <div className="text-center">
          <h1 className="font-heading font-bold text-3xl text-foreground mb-4 uppercase">
            {t(translations.servicePage.notFound)}
          </h1>
          <Link 
            to="/" 
            className="text-petrol-green hover:text-petrol-gold transition-colors font-heading uppercase tracking-wider"
          >
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative h-[50vh] min-h-[400px] overflow-hidden" data-testid="service-hero">
        <motion.img
          initial={{ scale: 1.1 }}
          animate={{ scale: 1 }}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
          <div className="container mx-auto">
            ...
            >
              <Link
                to="/"
                className="inline-flex items-center gap-2 text-white/70 hover:text-petrol-gold mb-4 transition-colors font-heading uppercase tracking-wider text-sm"
              >
                <ArrowLeft className="h-4 w-4" />
                {t(translations.servicePage.back)}
              </Link>
            </motion.div>
            <div className="overflow-hidden">
              <motion.h1
                ...
                className="font-heading font-bold text-3xl md:text-5xl lg:text-6xl text-white uppercase tracking-tight max-w-4xl"
              >
                {t(service.title)}
              </motion.h1>
            </div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-4xl mx-auto">
            ...
            >
              ...
              <p className="text-muted-foreground leading-relaxed text-lg mb-12 font-body whitespace-pre-line">
                {t(service.fullDesc)}
              </p>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-petrol-green-900 p-8 md:p-12 text-center"
            >
              <p className="text-white/80 mb-6 font-body text-lg">
                {t(translations.servicePage.interested)}
              </p>
              <Link
                to="/contacto"
            ...
            <div className="flex justify-between mt-12 pt-8 border-t border-stone-200">
              {prevService ? (
                <Link
                  ...
                  className="group flex items-center gap-3 text-petrol-green hover:text-petrol-gold transition-colors"
                >
                  <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
                  <div className="text-left">
                    <span className="block text-xs text-muted-foreground uppercase tracking-wider mb-1">
                      {t(translations.servicePage.prev)}
                    </span>
                    <span className="font-heading font-semibold uppercase tracking-wide text-sm hidden sm:block max-w-[200px] truncate">
                      {t(prevService.title)}
                    </span>
                  </div>
                </Link>
              ...
              {nextService ? (
                ...
                >
                  <div className="text-right">
                    <span className="block text-xs text-muted-foreground uppercase tracking-wider mb-1">
                      {t(translations.servicePage.next)}
                    </span>
                    <span className="font-heading font-semibold uppercase tracking-wide text-sm hidden sm:block max-w-[200px] truncate">
                      {t(nextService.title)}
                    </span>
